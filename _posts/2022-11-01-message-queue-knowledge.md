...
**实现方式**：
- 将 Topic 的读写队列数(queue)设置为 1
- Producer 和 Consumer 的并发数也要设置为 1 (读写都是单机)
...
**定义**：对于指定的一个 Topic，所有消息根据 Sharding Key(一般是业务唯一键) 进行区块分区。

**实现方式**：
- 同一个分区(queue)内的消息按照严格的 FIFO 顺序进行发布和消费
