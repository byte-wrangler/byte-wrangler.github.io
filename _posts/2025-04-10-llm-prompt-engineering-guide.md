...
在深入技巧之前,先理解一个核心问题:**为什么同样的需求,不同的提问方式会得到完全不同的结果?**

### 大模型的本质

之前的文章有说过，大模型可以看做是在玩"词语接龙"游戏。每一个字、每一句话,都会影响它接下来生成的内容。
...
经过大量实践,我发现 **COSTAR 框架**是一套非常系统化的方法,有助于建出高质量的提示词。

---

### 第一步：选择合适的表达框架

找一个合适的"容器",让大模型知道这是什么类型的任务。
...
有了框架,接下来就是如何更好地表达你的需求。这里我常用的两个技巧。

### 技巧 1: 善用"变量"

就是用特殊符号标记出来的占位符,让大模型知道这是一个需要替换的部分。
...
即使已经掌握了 COSTAR 框架和各种技巧,但要写出完美的提示词仍然不容易:
- 可能遗漏了某些重要信息
- 表达不够精确
- 没有考虑到边界情况
...
现在我是一个技术团队的负责人,需要让 AI 帮你为团队的 API 生成标准化的技术文档。这个任务涉及多个步骤,需要精心设计提示词才能得到高质量的输出。

---

### 第一步：选择表达框架 - 使用 COSTAR

首先,用 COSTAR 框架来组织用户提示词:
