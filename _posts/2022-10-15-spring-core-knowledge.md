...
#### 解决方案
结论：Spring Bean的二阶段创建 + Spring 三级缓存设计。
先实例化A的半对象，放入二级缓存，再实例化依赖的B对象，把依赖的B对象注入A属性，A就可以放入一级缓存了。
Spring AOP的引入导致代理对象的存在，所以实例化A对象的时候，不再是直接创建A的半对象放入二级缓存，而是创建一个A的生产工厂对象，
放入三级缓存，当B需要注入A的时候，从A的工厂对象中去判断A是否需要被代理，来确保是返回代理A对象还是A得原始半对象放入二级缓存。
